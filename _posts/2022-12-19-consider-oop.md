...
얼마전 회사에서 조영호님의 객체지향 프로그래밍에 관한 강연이 진행되었다. 
...
> 추가로 카드를 받을지 말지 결정이 "y", "n"로 이루어지고 있군요~
> 한번 간단한 상상을 해볼까요?
>
...
객체지향 프로그래밍이라는 개념이 왜 탄생했는가를 잠깐 생각해보면, 
결국 빠르게 코드를 고치고 정시에 퇴근하고 싶다는 선대 개발자들의 욕망이 담겨있다는 것을 알 수 있다. 
가장 큰 본질은 유지보수성을 위해서라는 것. 요즘 유행하는 아키텍쳐, 디자인 패턴을 따라 구현하는 것에 몰두되어선 안되겠다.
...
명확한 의도, 니즈 없이 사용하는 패턴들은 결국 빛 좋은 개살구일 뿐이다.
