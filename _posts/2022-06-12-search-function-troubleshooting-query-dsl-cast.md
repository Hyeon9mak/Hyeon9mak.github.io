...
인덱스 테이블이 없다고 해서 30만건도 안되는 데이터를 조회하는데 7초가 걸릴 리는 없을거 같았다. 
...
고심을 하다가 `ParentCareApply` 엔티티에서 `MomsitterUser`의 PK를 
...
- 곧장 경수, 민영님께 질문 드린건 잘한거 같다! 항상 질문하자!
