...
노가다성이 짙어보이지만, 엑셀 세로방향 그대로 복사 붙여넣기 후, 다중커서모드로 *domain_list.append("*, 다중커서모드에서 *End*키로 맨 뒤로 넘어가서 *")* 해주는 걸로 금방 리스트를 채울 수 있다.
...
아, 곧장 원본 엑셀 파일에 결과를 저장할 수 있으면 편하겠지만, 파일공유시스템에 막혀 직접 건드릴 수는 없었다. 때문에 복사 붙여넣기가 가능하도록 깔끔한 형태의 csv 파일을 만들 예정이라 pandas 모듈이 필요했다.
