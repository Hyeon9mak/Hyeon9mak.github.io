...
찾아낸 **head** 태그 내부에 포함되는 모든 소스코드들 중에서, 다음과 같은 경우를 따라 탐색을 진행한다.
- 연결재무제표
- 연 결 재 무 제 표
...
- 재무제표
- 재 무 제 표
위와 같은 경우를 통해 연결재무제표 때와 같이 개별 재무제표에 관한 정보를 때어오며, 둘 중 하나라도 발견하지 못 할 경우 **indi_flag**의 값을 1에서 0으로 변경시킨다.  
